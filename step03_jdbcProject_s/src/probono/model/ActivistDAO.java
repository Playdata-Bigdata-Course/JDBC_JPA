...
public class ActivistDAO {
	...
	public static ActivistDTO getActivist(String activistId) throws SQLException {
		...
		try {
			con = DBUtil.getConnection();
			pstmt = con.prepareStatement(sql.getProperty("activist.getActivist"));
			pstmt.setString(1, activistId);
			rset = pstmt.executeQuery();
			if (rset.next()) {
				activist = new ActivistDTO(rset.getString("activist_id"), rset.getString("major"),
						rset.getString("name"), rset.getString("password"));
			}
		} catch (SQLException s) {
			s.printStackTrace();
			throw s;
	...
	public static ArrayList<ActivistDTO> getAllActivists() throws SQLException {
		...
		try {
			con = DBUtil.getConnection();
			pstmt = con.prepareStatement(sql.getProperty("activist.All"));
			rset = pstmt.executeQuery();
			list = new ArrayList<ActivistDTO>();
			while (rset.next()) {
				list.add(new ActivistDTO(rset.getString("activist_id"), rset.getString("major"), rset.getString("name"),
