...
public class RunningEndView {
	...
	public static void ActivistListView(ArrayList allActivist){
		//? 데이터 수가 몇만건
		//하단 코드 적합 부적합 고민
		int length = allActivist.size();
		if( length != 0 ){
			for(int index = 0; index < length; index++){			
				System.out.println("검색정보 " + (index+1) + " - " + allActivist.get(index));
			}
		}
	}
	public static void ProbonoListView(ArrayList allProbono){
		//? 데이터 수가 몇만건
		//하단 코드 적합 부적합 고민
		int length = allProbono.size();
		if( length != 0 ){
			for(int index = 0; index < length; index++){			
				System.out.println("검색정보 " + (index+1) + " - " + allProbono.get(index));
			}
		}
	}
	//특정 프로젝트 출력 
